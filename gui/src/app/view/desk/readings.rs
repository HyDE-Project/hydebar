//! What each panel of the desk says, as plain text pairs.
//!
//! The canvas is a table of labels and values and nothing else, so the
...
impl Panel {
    /// A panel with rows, or nothing at all when the machine reported none.
...
/// Pushes a row when the machine answered for it.
...
pub(super) fn system(data: &SystemInfoData) -> Option<Panel> {
...
pub(super) fn network(data: &SystemInfoData) -> Option<Panel> {
...
/// The processor: how hard it is working, how hot and how fast.
pub(super) fn processor(data: &SystemInfoData) -> Option<Panel> {
    ...
    push(
        ...
        data.cpu_current_mhz.map(|mhz| {
            data.cpu_max_mhz
                .map_or_else(|| format!("{mhz} MHz"), |max| format!("{mhz} / {max} MHz"))
        })
    );
...
pub(super) fn graphics(data: &SystemInfoData) -> Option<Panel> {
...
pub(super) fn memory(data: &SystemInfoData) -> Option<Panel> {
    let mut rows = vec![
        (
            "in use".to_owned(),
