...
/// keeps its size and lets the last block run off the edge. A canvas nobody
/// can read has not fitted anything — it has only made the overflow smaller.
...
pub(super) fn ink_size(app: &App, id: Id, room: f32) -> f32 {
    ...
    let deepest = deepest_column(app, id, size, room);
...
/// The room the deepest of the three sections asks for, at the given size.
///
/// Asked of the sections as they are drawn — broken into runs where the room
/// calls for it — so a section that stands beside itself is measured by its
/// deepest run rather than by everything it holds.
fn deepest_column(app: &App, id: Id, size: f32, room: f32) -> f32 {
    let ink = blocks::Ink {
    ...
    [&left, &centre, &right]
        .into_iter()
        .map(|order| app.desk_section_room(order, id, ink, room))
