...
//! opens into, [`readings`] settles what each block says, [`room`] states the
//! measurements the whole canvas is laid out against, and [`fit`] sizes the
//! writing so the deepest column ends on the screen.
