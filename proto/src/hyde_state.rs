...
//!
//! `HyDE` keeps the choices the user made about the desktop — the theme in
//! force, the wallpaper shader, whether the colours are recoloured from the
//! wallpaper — in a small shell file that every `HyDE` script sources. The
//! installed themes are simply the directories under the `HyDE` configuration
//! root.
//!
