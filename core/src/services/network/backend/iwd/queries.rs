...
impl IwdDbus<'_> {
    ...
    pub async fn connectivity(&self) -> AppResult<Vec<String>> {
        ...
        for s in self.stations().await? {
            let state = s
                .state()
                .await
                .map_err(|e| bus_failure("Failed to get station state", &e))?;
            states.push(state);
        }
        Ok(states)
    }
    ...
    pub async fn wifi_device_present(&self) -> AppResult<bool> {
        ...
        for d in devices {
            if d.powered()
                .await
                .map_err(|e| bus_failure("Failed to get device powered state", &e))?
            {
                return Ok(true);
            }
        }
    ...
    pub async fn active_connections(&self) -> AppResult<Vec<(NetworkProxy, i16)>> {
        ...
        for (net, strength) in self.reachable_networks().await? {
            if net
                .connected()
                .await
                .map_err(|e| bus_failure("Failed to check network connected state", &e))?
            {
    ...
    pub async fn wireless_devices(&self) -> AppResult<Vec<DeviceProxy>> {
        ...
        for d in devices {
            if d.mode()
                .await
                .map_err(|e| bus_failure("Failed to get device mode", &e))?
                == "station"
            {
                devs.push(d);
            }
