//! The system tray: status notifier icons and the menus they publish.
//!
//! One folder, three rooms: [`module`] wires the module to the bar,
//! [`state`] owns the listener and folds messages in, [`view`] draws the
//! menus. The root holds the state the rooms share; the bar strip itself is
//! rendered by the GUI layer through `views::tray`.
