...
//! reports it, not because somebody listed it in a file. The configuration is
//! an override on top of that - it can pin the order, and it can turn a
//! readout off - and a machine that reports nothing simply draws nothing.
...
pub fn unavailable(indicator: &SystemIndicator, data: &SystemInfoData) -> Option<Unavailable> {
    match indicator {
        ...
        SystemIndicator::Disk(mount) => (!data.disks.iter().any(|(mounted, _)| mounted == mount))
...
///
/// A readout the machine cannot report is listed as unavailable together with
/// the reason, so nobody has to wonder why one they turned on shows nothing.
#[must_use]
