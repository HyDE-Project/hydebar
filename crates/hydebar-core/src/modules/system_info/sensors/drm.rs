...
//!
//! The monitoring subsystem carries temperatures only, so utilisation and
//! video memory come from the direct rendering device the same card owns. The
//! two are paired by the device the kernel links both to, which keeps a
//! machine with several cards from mixing one card's load with another's
//! temperature.
...
pub struct Card {
    ...
    /// Runtime power state, present once the driver allows the card to sleep.
...
impl Card {
    ...
    ///
    /// A card that powers down between uses is the normal state of switchable
    /// graphics, and reading it would be both meaningless and enough to wake
    /// it, so the panel leaves a sleeping card alone.
    #[must_use]
...
/// Reports whether a name addresses a card rather than one of its connectors.
///
/// The subsystem lists connectors beside the cards as `card0-HDMI-A-1`, and a
/// connector publishes neither load nor memory.
