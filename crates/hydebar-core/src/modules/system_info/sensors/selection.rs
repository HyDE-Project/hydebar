//! Picking one reading per role out of everything the machine publishes.
//!
//! The selection never touches the machine: it works on chips and input
//! labels, so the whole preference order is exercised against written-down
//! sensor sets rather than against whatever hardware the test runs on.

use super::catalog::{
    self, GpuPlacement, GpuVendor, cpu_chip_rank, cpu_input_rank, gpu_input_rank, gpu_vendor
};

/// Inputs of a chip that are not temperatures but tell one part from another.
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub struct ChipFacts {
    /// The chip publishes the northbridge voltage rail.
    ///
    /// The graphics driver exposes that rail only for a graphics block built
    /// into the processor, which is the one signal in the monitoring subsystem
    /// that separates it from a card.
    pub northbridge_voltage: bool,
    /// The chip publishes a fan reading.
    ///
    /// A graphics block built into the processor is cooled by the processor
    /// fan and owns none, so a fan of its own points at a card.
    pub fan:                 bool,
    /// The device sits directly on the root complex of its bus.
    ///
    /// Intel builds its graphics into the root complex and gives it the same
    /// address on every generation, while a card of theirs is reached through
    /// a bridge, so the address separates the two. [`None`] marks a device
...
impl ChipFacts {
    ...
    /// `<domain>:<bus>:<device>.<function>` the way the kernel writes it.
    #[must_use]
...
/// Reports whether an address names a device on the root complex of its bus.
...
pub struct ChipView<'a> {
    ...
    pub chip:   &'a str,
    /// Label of every temperature input, empty where the driver labels none.
    pub inputs: &'a [&'a str],
    pub facts:  ChipFacts
}
...
/// A chosen graphics reading together with what is known about the device.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct GpuPick {
    pub chip:      usize,
    pub input:     usize,
...
///
/// Cards driven by the vendor module and by its in-tree counterpart are always
/// add-in parts on the machines those drivers bind to. For the remaining
/// vendors the placement follows the extra inputs of the chip, and stays
/// unknown when nothing separates a tile on a card from a block inside the
/// processor, so an unknown placement is never presented as either one.
#[must_use]
...
///
/// The chip decides first, so a driver bound to the processor always beats the
/// board thermal zone that reports whichever node the firmware happens to
/// expose; the input decides second, so the package reading beats a core.
#[must_use]
...
///
/// `preferred` names the device a user pinned in the configuration, matched
/// against the vendor, the chip or the placement; an entry that matches
/// nothing is ignored rather than leaving the machine without a reading.
#[must_use]
...
mod tests {
    ...
    impl Sensors {
        ...
        /// Marks a chip as publishing the northbridge rail of an integrated
        /// part.
    ...
    /// Desktop machine: AMD processor, AMD card, storage and network noise.
    ...
    /// Laptop with switchable graphics: Intel processor and graphics block,
    /// NVIDIA card awake.
