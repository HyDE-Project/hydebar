...
//!
//! Enumeration walks the subsystem once and keeps the path of every input it
//! may later read, so a refresh is a read of the files that were already
//! chosen rather than another walk. The root is a parameter, so the walk is
//! exercised against a written-down directory tree instead of the machine the
...
pub struct Chip {
    ...
    ///
    /// Two subsystems describing the same piece of hardware agree on this
    /// path, which is how a temperature is paired with the utilisation the
    /// graphics driver publishes elsewhere.
    pub device: Option<PathBuf>
}

/// Every chip the monitoring subsystem publishes, in a stable order.
///
/// A machine without the subsystem, such as a virtual one, yields an empty
/// list: that is an ordinary state and not a failure, so nothing is logged.
#[must_use]
...
/// publishes millidegrees, yet a handful of drivers publish degrees, so a
/// reading small enough to be nonsense as millidegrees is taken as degrees.
/// The value is truncated rather than rounded, the same way every other
/// reading the module shows is, so two indicators never disagree by a degree.
