//! Which monitoring chip stands for which piece of hardware, and which of its
//! inputs to trust.
//!
//! Every decision here is a table lookup: adding a chip family or a vendor is
//! an entry in a list, never a condition spread over the module. Nothing in
//! this file reads the machine, so the whole selection can be exercised
//! against a written-down list of chips and input labels.
...
pub enum GpuVendor {
    ...
    Intel,
    Nvidia,
    /// A graphics block integrated into a system on a chip, such as the ones
    /// the ARM boards expose through their thermal zones.
    SystemOnChip
}

impl GpuVendor {
    /// Name the vendor is addressed by in the configuration and in the menu.
    #[must_use]
...
/// placement cannot be established, which in turn outranks the block built
/// into the processor, so a laptop with switchable graphics reports the card
/// whenever the card is awake.
...
impl GpuPlacement {
    ...
    ///
    /// Only the integrated block is tagged: a user looking at a machine with a
    /// card must never mistake the block inside the processor for it, while a
    /// machine with a single card gains nothing from a prefix.
    #[must_use]
...
/// Chip families whose readings stand for the processor, best tier first.
///
/// The first tier is a driver bound to the processor's own thermal registers,
/// so it reports the die itself. The second is the kernel package thermal
/// driver, which reports the same die through a coarser interface. The third
/// is the board thermal zone, which only tracks the processor on machines that
/// expose nothing better, and which a virtual machine may be the sole source
/// of.
...
/// Chip families that report a graphics processor, with the vendor behind
/// them.
///
/// `nvidia` is the chip the vendor kernel module registers once it grew
/// monitoring support; `nouveau` is the in-tree driver for the same cards.
/// `i915` and `xe` are the two Intel drivers, and `gpu thermal` is the zone
/// name the ARM boards give the block inside their system on a chip.
...
///
/// `tdie` is the die reading itself. `tctl` is the same die shifted by the
/// offset the cooling loop is tuned against, so it only stands in when the
/// driver publishes no die reading. `package id` and `physical id` are the two
/// spellings the kernel has used for the package reading, and `package` and
/// `cpu` cover the drivers that name it plainly.
...
///
/// A core runs hotter than the package it sits in and says nothing about the
/// rest of the chip, so one is read only when the driver publishes no package
/// reading at all.
const CPU_CORE_INPUTS: [&str; 3] = ["core", "tccd", "die"];

/// Graphics inputs, best first.
///
/// `junction` and `hotspot` are the two names for the hottest spot on the die,
/// which is the reading the vendor tools show. `die`, `gpu` and `core` are the
/// plain die readings of the remaining drivers. `edge` is measured at the rim
/// of the package and lags the die, so it stands in only when no die reading
/// exists. `mem` and `vram` measure the memory next to the die rather than the
/// die, and come last.
...
///
/// A driver that labels nothing still reports something useful when it is the
/// only input of a chip the tables recognise, which is how the vendor kernel
/// module publishes the graphics temperature.
pub const UNNAMED_INPUT_RANK: u8 = 200;

/// Folds a driver-supplied name into the spelling the tables carry.
///
/// Kernels differ in case, in whether they separate words with a space, a dash
/// or an underscore, and in the numeric suffix they append to the second
/// instance of a chip, so every spelling folds onto one entry instead of the
/// tables listing them all.
#[must_use]
...
///
/// A second chip of the same family arrives as `acpitz_1`, and a machine with
/// two processor packages arrives as `coretemp` twice, so the number belongs
/// to the instance rather than to the family.
#[must_use]
...
/// measures.
///
/// A driver that labels nothing leaves `tempN_label` absent, and the kernel
/// interface names the file itself `tempN`, so both forms mean the same.
#[must_use]
...
/// Rank of an input as the processor reading of its chip, lower being better.
#[must_use]
...
/// Rank of an input as the graphics reading of its chip, lower being better.
#[must_use]
