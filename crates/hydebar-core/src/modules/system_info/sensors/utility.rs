//! Optional vendor utilities, for the metrics the kernel keeps to itself.
//!
//! Everything the panel shows comes from the kernel first. A vendor utility is
//! consulted only where the kernel publishes nothing, it is looked up once
//! rather than on every refresh, it runs on a thread of its own so a slow or
//! wedged program never delays the bar, and its absence is an ordinary state
//! that costs nothing and is never logged as a fault.
//!
//! The AMD compute stack ships such a utility as well, and the panel does not
//! run it: the graphics driver already publishes the temperature, the load and
//! the video memory of every AMD part through the kernel, so spawning a
//! process would buy nothing. Adding a vendor is one entry in [`UTILITIES`]
//! together with the function that reads its output.
...
///
/// A utility costs a process, and on a laptop it can keep a card from powering
/// down, so it runs far more rarely than the bar refreshes and the bar shows
...
///
/// The NVIDIA driver publishes a monitoring chip only in its recent releases,
/// and never publishes the load, so its query interface is the one way to show
/// those numbers on an otherwise supported machine.
...
impl Feed {
    ...
    ///
    /// `gate` decides whether the device may be queried at all, which is how a
    /// sleeping card is left alone instead of being woken up for a reading.
...
///
/// A closed gate and a program that failed both mean the same to the bar: no
/// reading right now. The failure is written to the log once per feed, because
/// a device that is simply absent must not fill the journal every refresh.
fn poll<R, G>(
    utility: &Utility,
    runner: &R,
    gate: &G,
...
/// Reads one comma separated row of `temperature, load, memory used, memory
/// installed`.
///
/// Only the first row is read: a machine with two cards answers with one row
/// each, and the panel shows the card it selected rather than a sum. Fields
/// the driver cannot answer arrive as `[N/A]` and stay empty instead of being
/// shown as a zero.
