...
//!
//! The panel discovers the sensors once, keeps the files it settled on, and a
//! refresh reads exactly those files through one reused buffer: no walk of the
//! subsystem and no search for a chip. The set is rebuilt on a slow cadence,
//! and at once after a read fails, so a card that woke up appears and a card
//! that went to sleep disappears without anybody restarting the bar.
//!
//! A machine that publishes nothing - a virtual one, or a board whose drivers
//! expose no thermal registers - yields no readings at all. That is an
//! ordinary state: no indicator, no placeholder and nothing in the log.
...
///
/// Hardware comes and goes while the bar runs, and a walk of two small sysfs
/// trees is cheap, but not cheap enough to repeat every few seconds for a set
...
impl HardwareSensors {
    ...
    /// Starts, keeps or stops the vendor utility behind the selected device.
    ///
    /// It is started only where the kernel publishes neither a temperature nor
    /// a load for the device, and only when the program is installed, so a
    /// machine the kernel covers never spawns a process.
...
///
/// A driver that registers no monitoring chip still publishes a device, which
/// is what a machine reports while its vendor module lacks monitoring support
/// or while the reading comes from a vendor utility instead.
...
mod tests {
    ...
    impl Machine {
        ...
        /// Attaches a chip and a card to the same piece of hardware, the way
        /// the kernel links both subsystems to one device.
