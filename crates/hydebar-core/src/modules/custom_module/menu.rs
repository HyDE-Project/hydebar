...
//!
//! It is the native counterpart of the Waybar `menu-actions` map: every entry
//! a definition declares becomes a row running its command, without the
//! separate GTK menu file Waybar needs to name the rows.
...
///
/// `on_select` turns the pressed entry into the message the caller reacts to,
/// so the module stays unaware of how the command is run and of how the menu
/// surface is dismissed.
///
/// Entries carry their glyph verbatim rather than through the icon theme: an
/// entry names the glyph the way the module itself does, so there is no named
/// icon to resolve.
///
/// Entries missing a label or a command are dropped by
/// [`CustomModuleDef::menu_entries`], leaving an empty column for a definition
/// without a usable entry.
