...
impl App {
    ...
    fn single_module_wrapper(
        ...
        module.map(|(content, actions)| match actions.is_inert() {
            false => {
        ...
        let modules = group
            ...
            .filter_map(|module| {
                self.get_module_view(module, id, opacity)
                    .map(|(content, action)| (content, self.module_actions(module, action)))
            })
            .collect::<Vec<_>>();
        ...
        if modules.is_empty() {
            ...
            Some({
                let group = Row::with_children(
                    modules
                        .into_iter()
                        .map(|(content, actions)| match actions.is_inert() {
                            false => {
                                let button = position_button(
                                    container(content)
                                        .align_y(Alignment::Center)
